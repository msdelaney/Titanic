...
// Would likely do in python if not trying to get JS experience
  ...
  var fare_scalem = d3.scale.log().domain([3, 550]).range([height/2 - padding, title_height]);
  var age_scalef = d3.scale.linear().domain([-2, 82]).range([padding, width-leg_width]);
  var fare_scalef = d3.scale.log().domain([3, 550]).range([height - padding,height/2 + title_height]);
  ...
  var shapes = svg.selectAll(".shape")
   ...
   // .on("mouseover",tip.show)
    ...
    .text("Fare in Dollars")
     ...
     svg.append("text")
        .attr("x", (width / 2))             
        .attr("y", height /2 + (title_height /2))
        .attr("text-anchor", "middle")  
        .style("font-size", "16px") 
        .style("text-decoration", "underline")  
